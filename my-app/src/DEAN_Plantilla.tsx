...
const WorkflowChips = () => {
  ...
  // highlight range: first "APO" -> first "Dean" after that
  const startIdx = steps.findIndex((s) => s === "APO");
  const endIdx = steps.indexOf("Dean", Math.max(0, startIdx));

  return (
    <div className="flex flex-wrap items-center gap-2 mt-3">
      {steps.map((step, i) => {
        const isGreen = startIdx !== -1 && endIdx !== -1 && i >= startIdx && i <= endIdx;
        const dashGreen = startIdx !== -1 && endIdx !== -1 && i >= startIdx && i < endIdx;
        return (
          <React.Fragment key={`${step}-${i}`}>
            ...
            {i < steps.length - 1 && (
              <span className={dashGreen ? "text-emerald-600" : "text-gray-400"}>—</span>
            )}
          </React.Fragment>
        );
      })}
...
function DeanTabs() {
  const items = [
    { to: "/dean/plantilla", label: "Plantilla", icon: FileSpreadsheet },
    { to: "/dean/classretention", label: "Class Retention", icon: ClipboardList },
  ];
...
export default function Dean_Plantilla() {
  return (
