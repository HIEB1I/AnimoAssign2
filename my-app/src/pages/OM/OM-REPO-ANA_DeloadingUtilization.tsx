...
/* --- UI bits copied/adapted to match the styled page --- */
...
const TypeBreakdownBar = ({ activeByType }: { activeByType: Record<string, number> }) => {
      ...
      <div className="mb-2 h-3 w-full overflow-hidden rounded-full bg-gray-100 flex">
        {segments.map(({ key, value }) => {
          ...
          return (
            <div
              key={key}
              className={cls(TYPE_PALETTE[key as keyof typeof TYPE_PALETTE].bar, "h-3")}
              style={{ width: `${width}%` }}
            />
          );
        })}
...
/* --- Functional data from your “good functionality” snippet --- */
type ActiveRow = { id: number; faculty: string; type: string; units: number; notes?: string };

export default function OM_REPO_ANA_DeloadingUtilization() {
  // Same initial data & behavior as the good (functional) version
  const [active, setActive] = useState<ActiveRow[]>([
    { id: 1, faculty: "Faculty #1", type: "Administrative", units: 6, notes: "Dept. Chair — ST Department" },
    { id: 2, faculty: "Faculty #2", type: "Research",       units: 5, notes: "Dissertation Writing" },
    { id: 3, faculty: "Faculty #3", type: "Extension",       units: 3, notes: "External Project Support" },
  ]);

  const metrics = useMemo(() => {
    const totalApproved = active.reduce((s, r) => s + r.units, 0);
    const facultyWithActive = active.length;

    // No carry state: utilization = 100% if there are used units, else 0%
    ...
    for (const r of active) activeByType[normalizeType(r.type)] += r.units;
  ...
  return (
    <AppShell>
      <main className="w-full px-8 py-8">
        ...
        {/* Filter bar with Back — keep the “styled” page look */}
        <div className="mb-6 flex flex-wrap items-center gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          ...
          {/* Right side intentionally empty to match the bar layout without adding wrong functionality */}
          <div className="ml-auto" />
        </div>

        {/* KPI cards, but using your (good) metrics */}
        <div className="grid gap-4 md:grid-cols-3 mb-2">
          <KpiCard title="Total Approved Units (This Term)" value={metrics.totalApproved} />
          <KpiCard title="Faculty with Active Deloadings" value={metrics.facultyWithActive} />
          <KpiCard
            title="Utilization Rate"
            value={`${metrics.utilization}%`}
            subtitle={`${metrics.usedUnits} used of ${metrics.denom || 0} available`}
          />
        </div>

        {/* Type breakdown (from functional code) */}
        <TypeBreakdownBar activeByType={metrics.activeByType} />

        {/* Department table with editable Units (from functional code), re-styled to match */}
        <Section title="Faculty Deloading — Department Breakdown">
          ...
          {/* Save button (no-op, same as your good code) */}
