              ...
              <tbody className="text-gray-900">
                {byFaculty.map(([faculty, items]) => (
                  <React.Fragment key={faculty}>
                    ...
                    {items.map((r, i) => (
                      <tr key={r.id} className={cls("bg-white", "[&>td]:border-t [&>td]:border-gray-100", i === 0 && manyGroups ? "" : "")}>
...
export default function OM_LoadAssignment() {
  ...
  const loadSample = () => {
    setRows([
      ...
      },
      {
        ...
        status: "Conflict",
        conflictNote:
          "Dr. Cheng, Danny is already scheduled for three consecutive classes.",
      },
    ]);
    setMode("run");
    setApproved(false);
  ...
  /* ---- Editability in one place ---- */
  const getEditFlags = (r: Row) => {
    const editAll = !!r.editable; // new line only
    const editSchedule = editAll || isRun; // schedule-only when mode is run
    return {
      course: editAll,
      title: editAll,
      units: editAll,
  ...
  /* ---- Generic table cell: input or display ---- */
  const Cell = ({
    editable,
    value,
    onChange,
