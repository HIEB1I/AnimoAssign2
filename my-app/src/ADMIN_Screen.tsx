...
/* Name helpers: ensure “Lastname, Firstname M.” everywhere */
...
const normalizeFullName = (raw: string) => {
  if (!raw) return raw;
  // If already "Last, First Mi"
  if (raw.includes(",")) {
    const [last, rest] = raw.split(",", 2);
    const parts = (rest || "").trim().split(/\s+/).filter(Boolean);
    const first = parts[0] || "";
    const mi = (parts[1] || "").replace(".", "");
    return formatName(last.trim(), first, mi);
  }
  // Else assume "First [Mi] Last"
...
function Dropdown({
  value, onChange, options, className = "w-full",
  ...
  return (
    <div className={cls("relative", className)} onKeyDown={onKey}>
      <button
        ref={btnRef} onClick={() => setOpen(v => !v)} aria-haspopup="listbox" aria-expanded={open}
...
const TimestampCell = ({ ts }: { ts: string }) => {
  const [d, t] = ts.split(" ");
  return <div className="leading-tight"><div className="text-[13px] text-slate-700">{d}</div><div className="text-[11px] text-gray-500">{t}</div></div>;
};

/* ----------------------- Data & types ----------------------- */
type UserStatus = "Active" | "Leave" | "Sabbatical" | "Inactive";
type UserRole =
  | "APO" | "Department Chair" | "Faculty-FT" | "Faculty-PT"
  | "Office Assistant" | "Office Manager" | "Provost’s Office" | "Student";
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-emerald-900/30">
      ...
      {notifOpen && (
        <div
          ref={notifRef}
          className="absolute right-5 top-14 z-30 w-96 rounded-xl border border-neutral-200 bg-white shadow-lg"
        >
          <div className="px-4 py-3 border-b border-neutral-200 font-semibold text-emerald-700">Notifications</div>
          <div className="max-h-96 overflow-y-auto">
            {sortedNotifs.length > 0 ? (
              sortedNotifs.map(n => (
                <div key={n.id} className="px-4 py-3 border-b border-neutral-100 last:border-0">
...
/* ----------------------- Modal ----------------------- */
function Modal({
  open, onClose, children, width = "max-w-4xl",
  ...
  return (
    <div className="fixed inset-0 z-40">
      ...
      <div className="absolute inset-0 grid place-items-center p-4">
        <div ref={boxRef} className={cls("w-full rounded-2xl bg-white shadow-2xl", width)}>
          {children}
        </div>
      </div>
    </div>
...
type AddUserForm = {
  ...
  email: string; status: UserStatus | ""; role: UserRole | ""; department: string | "";
...
const emptyAdd: AddUserForm = {
  lastName: "", firstName: "", middleInitial: "", email: "", status: "", role: "", department: "",
...
export default function ADMIN_UserAndLogs() {
  ...
  const [qUsers, setQUsers] = useState(""), [role, setRole] = useState<"All Roles" | UserRole>("All Roles");
  ...
  const usersFiltered = useMemo(() => {
    ...
    return users.filter(u =>
      (!t || normalizeFullName(u.fullName).toLowerCase().includes(t) || u.email.toLowerCase().includes(t)) &&
      (role === "All Roles" || u.role === role) &&
      (!activeOnly || u.status === "Active")
    );
  }, [qUsers, role, activeOnly, users]);

  const [qLogs, setQLogs] = useState(""), [action, setAction] = useState<"All Actions" | "User Login" | "Role Update" | "Password Reset" | "Permission Grant" | "Data Export" | "System Backup" | "User Creation" | "User Deactivation">("All Actions");
  const LOGS = [
  ...
  const logs = LOGS.filter(r => (action === "All Actions" || r.action === action) &&
  ...
  const submitAdd = () => {
    ...
    const next: User = {
      ...
      status: (addForm.status || "Active") as UserStatus,
      role: (addForm.role || "Student") as UserRole,
      department: (addForm.department || "None"),
  ...
  /* ---------- Delete flow ---------- */
  const openDelete = (u: User) => { setDeleteOpen({ ...u, fullName: normalizeFullName(u.fullName) }); setMenuOpenId(null); };
  const confirmDelete = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-gray-50 text-slate-900" style={{ scrollbarGutter: "stable both-edges" }}>
      ...
      <main className="mx-auto w-full max-w-none px-4 sm:px-6 lg:px-8 py-6 space-y-8">
        {/* ---------------- Users ---------------- */}
        <section className="rounded-xl border border-neutral-200 bg-white">
          ...
          <div className="flex flex-wrap items-center gap-3 px-6 py-4">
            <div className="relative flex-1 min-w-[360px] max-w-[920px]">
              <Search className="absolute left-3 top-2.5 h-4 w-4 text-neutral-400" />
              <input
                value={qUsers} onChange={e => setQUsers(e.target.value)}
                placeholder="Search by name..."
                className="h-10 w-full rounded-lg border border-neutral-200 pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
            <Dropdown value={role} onChange={v => setRole(v as any)} options={ROLE_OPTIONS} className="w-56 text-left" />
            <div className="inline-flex rounded-lg border border-neutral-200 bg-white p-1">
              <button onClick={() => setActiveOnly(false)} aria-pressed={!activeOnly}
                className={cls("h-10 rounded-md px-4 text-sm font-medium transition", !activeOnly ? "bg-green-600 text-white" : "text-neutral-800 hover:bg-neutral-50")}>
                Show All Status
              </button>
              <button onClick={() => setActiveOnly(true)} aria-pressed={activeOnly}
                className={cls("h-10 rounded-md px-4 text-sm font-medium transition", activeOnly ? "bg-green-600 text-white" : "text-neutral-800 hover:bg-neutral-50")}>
                Show Active
              </button>
            </div>
          ...
          <div className="overflow-x-auto">
            <div className="max-h-[330px] overflow-y-auto">
              <table className="min-w-full table-fixed text-sm">
                <colgroup>
                  <col className="w-[260px]" /><col /><col className="w-[9.5rem]" /><col className="w-[9.5rem]" />
                  <col className="w-[9.5rem]" /><col /><col /><col className="w-[92px]" />
                </colgroup>
                <thead>
                  <tr className="text-left text-xs text-neutral-500">
                    ...
                    {["Status","Role","Department"].map(h => (
                      <th key={h} className="px-3 py-3 font-medium"><div className="flex justify-center"><span className="inline-block min-w-[7.5rem] text-center">{h}</span></div></th>
                    ))}
                ...
                <tbody>
                  {usersFiltered.map(u => (
                    <tr key={u.id} className="border-t border-neutral-200 align-middle">
                      ...
                      <td className="px-3 py-4"><div className="flex justify-center">
                        <Pill minW="min-w-[7.5rem]" tone={u.status === "Active" ? "emerald" : u.status === "Leave" ? "blue" : u.status === "Sabbatical" ? "amber" : "neutral"}>{u.status}</Pill>
                      </div></td>
                      <td className="px-3 py-4"><div className="flex justify-center"><Pill minW="min-w-[7.5rem]">{u.role}</Pill></div></td>
                      <td className="px-3 py-4"><div className="flex justify-center"><DeptTag code={u.department} /></div></td>
                  ...
                  {usersFiltered.length === 0 && (
                    <tr><td colSpan={8} className="px-6 py-10 text-center text-sm text-neutral-500">No users found for the selected filters.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <section className="rounded-2xl border border-gray-100 bg-white shadow-sm">
          ...
          <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><Search className="h-4 w-4" /></span>
              <input
                ...
                placeholder="Search by name, action, details..." value={qLogs} onChange={e => setQLogs(e.target.value)}
              />
            </div>
            <Dropdown
              value={action} onChange={v => setAction(v as any)}
              options={["All Actions","Data Export","Password Reset","Permission Grant","Role Update","System Backup","User Creation","User Deactivation","User Login"]}
          ...
          <div className="p-4">
            <div className="grid grid-cols-[13%_22%_40%_15%_10%] items-center px-3 py-2 text-xs font-semibold text-gray-500">
              <div>User</div>
              <div className="flex justify-center"><span className="inline-block min-w-[8.5rem] text-center">Action</span></div>
              <div>Details</div><div>Timestamp</div>
              <div className="flex justify-center"><span className="inline-block min-w-[5.5rem] text-center">Status</span></div>
            </div>
            ...
            <div className="max-h-[300px] overflow-y-auto">
              {logs.map((r, i) => (
                <div key={r.id} className={cls(GRID, "px-3 py-3 text-sm", i !== logs.length - 1 && "border-b border-gray-200")}>
                  <div><button className="text-emerald-700 hover:underline">{normalizeFullName(r.user)}</button></div>
                  <div className="flex justify-center"><Pill minW="min-w-[8.5rem]">{r.action}</Pill></div>
                  <div className="text-gray-600">{r.details}</div>
                  <div><TimestampCell ts={r.timestamp} /></div>
                  <div className="flex justify-center">
                    <Pill minW="min-w-[5.5rem]" tone={r.action === "User Login" ? "emerald" : r.action === "Password Reset" ? "red" : "amber"}>
                      {r.action === "User Login" ? "Active" : r.action === "Password Reset" ? "Failed" : "Warning"}
                    </Pill>
                  </div>
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 gap-5">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div>
                ...
                <input value={addForm.lastName} onChange={e => setAddForm(f => ({...f, lastName: e.target.value}))}
                  placeholder="Last Name" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30" />
              </div>
              <div className="md:mt-[26px]">
                <input value={addForm.firstName} onChange={e => setAddForm(f => ({...f, firstName: e.target.value}))}
                  placeholder="First Name" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30" />
              </div>
              <div className="md:mt-[26px]">
                <input value={addForm.middleInitial} onChange={e => setAddForm(f => ({...f, middleInitial: e.target.value}))}
                  placeholder="Middle Initial" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30" />
              </div>
            </div>
            ...
            <div>
              ...
              <input value={addForm.email} onChange={e => setAddForm(f => ({...f, email: e.target.value}))}
                placeholder="rafael.cabredo@dlsu.edu.ph" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30" />
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  <select value={addForm.status} onChange={e => setAddForm(f => ({...f, status: e.target.value as any}))}
                    className="w-full appearance-none rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <div className="relative">
                  <input
                    type={pwVis.add1 ? "text" : "password"}
                    value={addForm.password}
                    onChange={e => setAddForm(f => ({...f, password: e.target.value}))}
                    className="w-full rounded-lg border border-gray-300 px-3 py-2 pr-9 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30"
                  />
                  <button type="button" onClick={() => setPwVis(v => ({...v, add1: !v.add1}))} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500">
                    {pwVis.add1 ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                  </button>
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <input
                    type={pwVis.add2 ? "text" : "password"}
                    value={addForm.confirmPassword}
                    onChange={e => setAddForm(f => ({...f, confirmPassword: e.target.value}))}
                    className="w-full rounded-lg border border-gray-300 px-3 py-2 pr-9 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30"
                  />
                  <button type="button" onClick={() => setPwVis(v => ({...v, add2: !v.add2}))} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500">
                    {pwVis.add2 ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="grid grid-cols-1 gap-5 mt-4">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div className="md:mt-[26px]">
                  <input value={editForm.firstName} onChange={e => setEditForm(f => f && ({...f, firstName: e.target.value}))}
                    placeholder="First Name" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30" />
                </div>
                <div className="md:mt-[26px]">
                  <input value={editForm.middleInitial} onChange={e => setEditForm(f => f && ({...f, middleInitial: e.target.value}))}
                    placeholder="Middle Initial" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div>
                  <label className="mb-1 block text-sm font-semibold">Role</label>
                  <div className="relative">
                    <select value={editForm.role} onChange={e => setEditForm(f => f && ({...f, role: e.target.value as any}))}
                      className="w-full appearance-none rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30">
                      <option value="">-- Select an option --</option>
                      {ROLE_OPTIONS.filter(r => r !== "All Roles").map(r => <option key={r} value={r}>{r}</option>)}
                    </select>
                ...
                <div>
                  <label className="mb-1 block text-sm font-semibold">Department</label>
                  <div className="relative">
                    <select value={editForm.department} onChange={e => setEditForm(f => f && ({...f, department: e.target.value}))}
                      className="w-full appearance-none rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30">
                      <option value="">-- Select an option --</option>
                      {DEPT_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  <label className="mb-1 block text-sm font-semibold">OLD Password</label>
                  <div className="relative">
                    <input
                      type={pwVis.editOld ? "text" : "password"}
                      value={editForm.oldPassword}
                      onChange={e => setEditForm(f => f && ({...f, oldPassword: e.target.value}))}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 pr-9 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30"
                    />
                    <button type="button" onClick={() => setPwVis(v => ({...v, editOld: !v.editOld}))} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500">
                      {pwVis.editOld ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                    </button>
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    <input
                      type={pwVis.editNew ? "text" : "password"}
                      value={editForm.newPassword}
                      onChange={e => setEditForm(f => f && ({...f, newPassword: e.target.value}))}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 pr-9 text-sm outline-none focus:ring-2 focus:ring-emerald-500/30"
                    />
                    <button type="button" onClick={() => setPwVis(v => ({...v, editNew: !v.editNew}))} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500">
                      {pwVis.editNew ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                    </button>
                  </div>
                </div>
              </div>
        ...
        {deleteOpen && (
          <div className="p-6 sm:p-8">
            ...
            <div className="mt-2 flex justify-center">
              <div className="grid h-16 w-16 place-items-center rounded-full border-2 border-red-400 text-red-500 text-2xl font-bold">
                X
              </div>
            </div>
            ...
            <div className="mt-4 text-center">
              ...
              <p className="mt-2 text-gray-400">
                ...
                <span className="font-medium text-gray-600">{normalizeFullName(deleteOpen.fullName)}</span>? This process cannot be undone.
              </p>
            </div>
