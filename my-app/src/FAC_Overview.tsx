          ...
          <div ref={wrapperRef} className="relative">
            <button onClick={() => setMenuOpen((o) => !o)} className="group flex items-center gap-3 rounded-lg px-2 py-1 hover:bg-white/10">
              ...
              <span className="leading-tight text-left">
                ...
                <div className="text-[12px] opacity-90">{role} | {department}</div>
              </span>
            </button>
      ...
      {cards.map(({ title, value, subtitleLeft, subtitleRight, progress }) => (
        <div key={title} className="rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 flex items-center justify-between text-[11px] text-neutral-600">
            <span>{subtitleLeft}</span><span>{subtitleRight}</span>
          </div>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-neutral-200">
            <div className="h-full bg-emerald-700" style={{ width: `${Math.max(0, Math.min(100, progress))}%` }} />
          </div>
        </div>
      ))}
    </div>
...
type TL = {
  ...
  items: {
    code: string; sec: string; mode: "Hybrid" | "Online" | "Onsite"; status: "Confirmed" | "Pending";
    title: string; time: string; campus: string; room: string | "Online"; students: number; units: number;
  }[];
};

const SAMPLE: TL[] = [
  { day: "Monday", items: [
    { code: "CSMODEL", sec: "S12", mode: "Hybrid", status: "Confirmed", title: "Data Modeling and Database Design", time: "7:30–9:00 AM", campus: "Manila", room: "Online", students: 35, units: 3 },
    { code: "CSMODEL", sec: "S13", mode: "Hybrid", status: "Confirmed", title: "Data Modeling and Database Design", time: "9:15–10:45 AM", campus: "Manila", room: "Online", students: 35, units: 3 },
  ]},
  { day: "Thursday", items: [
    { code: "CSMODEL", sec: "S12", mode: "Hybrid", status: "Confirmed", title: "Data Modeling and Database Design", time: "7:30–9:00 AM", campus: "Manila", room: "GK210", students: 35, units: 3 },
    { code: "CSMODEL", sec: "S13", mode: "Hybrid", status: "Confirmed", title: "Data Modeling and Database Design", time: "9:15–10:45 AM", campus: "Manila", room: "GK211", students: 35, units: 3 },
  ]},
  { day: "Saturday", items: [
    { code: "CCPROG3", sec: "S12", mode: "Online", status: "Confirmed", title: "Object-Oriented Programming", time: "9:00 AM – 12:00 PM", campus: "Laguna", room: "Online", students: 22, units: 3 },
  ]},
];

/* ---------- generic dropdown ---------- */
function Dropdown({ value, onChange, options, className = "w-full", placeholder = "— Select an option —" }: {
  ...
  return (
    <div className={cls("relative", className)} onKeyDown={onKey}>
      ...
      {open && (
        <div ref={listRef} role="listbox" className="absolute z-20 mt-2 w-full max-h-80 overflow-auto rounded-2xl border border-gray-300 bg-white shadow-lg">
          {options.map((opt, i) => (
            <button key={opt} role="option" aria-selected={value === opt} onMouseEnter={() => setHover(i)} onClick={() => { onChange(opt); setOpen(false); btnRef.current?.focus(); }}
...
const TIME_SLOTS = ["7:30 – 9:00 AM","9:15 – 10:45 AM","11:00 – 12:30 PM","12:30 – 2:15 PM","2:30 – 4:00 PM","4:15 – 5:45 PM","6:00 – 7:30 PM","7:45 – 9:00 PM"];
const ALL_DAYS: TL["day"][] = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];

function ChangeRequestModal({ open, onClose, context }: { open: boolean; onClose: () => void; context: { day: TL["day"]; item: TL["items"][number] } | null; }) {
  const normTime = (s: string) => s.replace(/[\s\u2013-]/g, "").replace(/\./g, "").toLowerCase();
  const timeOptions = useMemo(() => (!context ? TIME_SLOTS : TIME_SLOTS.filter((t) => normTime(t) !== normTime(context.item.time))), [context]);
  const [choices, setChoices] = useState<ChangeKind[]>([]);
  const [selTime, setSelTime] = useState("");
  const [selDay, setSelDay] = useState<TL["day"] | "">("");
  const [remarks, setRemarks] = useState("");

  useEffect(() => { if (!open) { setChoices([]); setSelTime(""); setSelDay(""); setRemarks(""); } }, [open]);
  const toggle = (label: ChangeKind) => setChoices((prev) => label === "I will be on leave" ? [label] : prev.includes(label) ? prev.filter((c) => c !== label && c !== "I will be on leave") : [...prev.filter((c) => c !== "I will be on leave"), label]);

  const mustTime = choices.includes("Change class time");
  const mustDay = choices.includes("Change class day");
  const disabled = choices.length === 0 || (mustTime && !selTime) || (mustDay && !selDay) || !remarks.trim();
      ...
      <div className="w-full max-w-3xl rounded-2xl bg-white p-6 shadow-2xl">
        <div className="mb-4 flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-neutral-500">{context.item.code} {context.item.sec} • {context.day} • {context.item.time}</p>
          </div>
          <button className="rounded-full p-1 hover:bg-neutral-100" onClick={onClose} aria-label="Close"><X className="h-5 w-5" /></button>
        </div>

        <div className="space-y-3">
          <label className="block text-sm font-medium text-neutral-700">Change</label>
          <div className="flex flex-wrap gap-2">
            {["Change class time","Change class day","I will be on leave"].map((opt) => (
              <button key={opt} onClick={() => toggle(opt as ChangeKind)} className={cls("rounded-lg border px-3 py-2 text-sm", choices.includes(opt as ChangeKind) ? "border-emerald-600 bg-emerald-50 text-emerald-700" : "border-neutral-300 bg-white hover:bg-neutral-50")}>{opt}</button>
            ))}
          </div>
          ...
          {mustDay && (
            <div className="mt-2">
              ...
              <Dropdown value={selDay} onChange={(v) => setSelDay(v as TL["day"])} options={dayOptions} placeholder="— Select a day —" />
            </div>
          )}
          ...
          {!!choices.length && (
            <div className="mt-2">
              ...
              <textarea rows={4} className="w-full resize-y rounded-lg border border-neutral-300 p-2 text-sm" placeholder="Provide context for this request…" value={remarks} onChange={(e) => setRemarks(e.target.value)} />
            </div>
          )}
        </div>

        <div className="mt-6 flex items-center justify-end gap-2.5">
          <button onClick={onClose} className="inline-flex h-9 items-center justify-center rounded-[12px] border border-neutral-200 bg-neutral-100 px-4 text-sm text-slate-900 shadow-sm hover:bg-neutral-200/70 active:translate-y-[0.5px]">Cancel</button>
          <button disabled={disabled} onClick={() => { console.log("SUBMIT", { context, choices, newTime: selTime || null, newDay: selDay || null, remarks }); onClose(); }}
            className={cls("inline-flex h-9 items-center gap-2 rounded-[12px] px-4 text-sm text-white shadow","bg-[#1F7A49] hover:brightness-[1.06] active:translate-y-[0.5px] focus:outline-none focus:ring-2 focus:ring-emerald-600/40", disabled && "opacity-60 cursor-not-allowed")}
...
/* ---------- calendar helpers ---------- */
const TIME_BANDS = ["7:30 – 9:00","9:15 – 10:45","11:00 – 12:30","12:30 – 14:15","14:30 – 16:00","16:15 – 17:45","18:00 – 19:30","19:45 – 21:00"];
...
function placeItems(data: TL[]): Placed[] {
  ...
  const findRowSpan = (time: string) => {
    ...
    const anchors: Array<{ rx: RegExp; row: number }> = [
      { rx: /\b7:30\b/, row: 0 }, { rx: /\b9:15|\b9:00\b/, row: 1 }, { rx: /\b11:00\b/, row: 2 },
      { rx: /\b12:30\b/, row: 3 }, { rx: /\b14:30|\b2:30\b/, row: 4 }, { rx: /\b16:15|\b4:15\b/, row: 5 },
      { rx: /\b18:00|\b6:00\b/, row: 6 }, { rx: /\b19:45|\b7:45\b/, row: 7 },
    ];
    const hit = anchors.find((a) => a.rx.test(t));
    return { row: hit?.row ?? 0, span: 1 };
  };
  const out: Placed[] = [];
  data.forEach((d) => d.items.forEach((it) => { const { row, span } = findRowSpan(it.time); out.push({ day: d.day, row, rowSpan: Math.min(TIME_BANDS.length - row, span), data: it }); }));
  return out;
}

const ClassBlock = ({ onClick, it }: { onClick?: () => void; it: TL["items"][number] }) => (
  <button onClick={onClick} className={cls("flex w-full flex-col items-center justify-center rounded-xl border shadow-sm","border-emerald-200 bg-emerald-50/90 hover:bg-emerald-50")} title={`${it.code} • ${it.sec} | ${it.room} • ${it.mode}`}>
      ...
      <div className="rounded-xl border border-neutral-200 bg-white p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="inline-flex gap-2">
            {["List", "Calendar"].map((label) => {
              ...
              return (
                <button key={label} type="button" onClick={() => setTab(label as typeof tab)} aria-pressed={active}
                  className={cls("inline-flex h-9 min-w-[120px] items-center justify-center rounded-lg px-4 text-sm font-medium", active ? "bg-emerald-600 text-white" : "bg-neutral-100 text-neutral-700 hover:bg-neutral-200")}
                >
                  {label}
                </button>
              );
        ...
        {tab === "Calendar" ? (
              ...
              <div className="relative grid grid-cols-[140px_repeat(6,1fr)]" style={{ gridAutoRows: "84px" }}>
                {TIME_BANDS.map((band, r) => (
                  <React.Fragment key={band}>
                    <div className="flex items-center justify-center border-r border-neutral-300 bg-neutral-50 px-2 text-center text-[13px]" style={{ gridColumn: 1, gridRow: r + 1 }}>{band}</div>
                    {DAY_ORDER.map((_, c) => (
                      <div key={`${c}-${r}`} className="border border-neutral-300" style={{ gridColumn: c + 2, gridRow: r + 1 }} />
                    ))}
                  </React.Fragment>
                ))}

                {placed.map((p, i) => (
                  <div key={`blk-${i}`} className="p-2" style={{ gridColumn: DAY_ORDER.indexOf(p.day) + 2, gridRow: `${p.row + 1} / span ${p.rowSpan}` }}>
          ...
          <div className="space-y-6">
            {SAMPLE.map((day) => (
              <div key={day.day}>
                <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-neutral-800"><CalIcon className="h-4 w-4" /> {day.day}</div>
                <div className="space-y-3">
                  {day.items.map((it, idx) => (
                    <div key={idx} className="rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
                      <div className="flex items-start justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <Tag tone="emerald"><Check className="h-3 w-3" />{it.status}</Tag>
                          </div>
                      ...
                      <div className="mt-3 grid grid-cols-1 items-center gap-3 text-sm text-neutral-600 sm:grid-cols-6">
                        <div className="flex items-center gap-2"><Clock className="h-4 w-4" />{it.time}</div>
                        <div className="flex items-center gap-2"><CalendarDays className="h-4 w-4" />{day.day}</div>
                        <div className="flex items-center gap-2"><MapPin className="h-4 w-4" />{it.campus}</div>
                        <div className="flex items-center gap-2"><Monitor className="h-4 w-4" />{it.room}</div>
                        <div className="flex items-center gap-2"><Users2 className="h-4 w-4" />{it.students} students</div>
                        <div className="flex justify-end">
                          <button title="Request for change" className="grid h-10 w-10 place-items-center rounded-full bg-emerald-700 text-white shadow-[0_2px_6px_rgba(0,0,0,0.2)] hover:brightness-110 active:scale-95" onClick={() => setModal({ day: day.day, item: it })}>
